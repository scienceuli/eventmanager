...
class Home(BaseModel):
    ...
    about_us = RichTextField("Über uns", blank=True, config_name="short")
...
class Event(BaseModel, HitCountMixin):
    ...
    show_in_all_events = models.BooleanField(
        verbose_name="in Gesamtübersicht zeigen?", default=True
    )
    label = models.CharField(
        max_length=64,
        verbose_name="Kurzname",
    ...
    def get_balance(self):
        if not self.costs:
            return round(Decimal(0), 2)
        balance = self.get_total_income_for_event() - self.costs
        return balance
