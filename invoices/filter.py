...
class InvoiceEventListFilter(admin.SimpleListFilter):
    title = "Rechnungen"
    ...
    def queryset(self, request, queryset):
        if self.value():
            return queryset.filter(order__items__event__id=self.value())
        return queryset


class InvoicesYearQuarterFilter(admin.SimpleListFilter):
    ...
    def queryset(self, request, queryset):
        ...
        if value:
            ...
            return queryset.filter(invoice_date__range=[start_date, end_date])
        return queryset